...
def preformat_OGB_PCQM4Mv2(dataset_dir, name):
    """
    加载 PCQM4Mv2 数据集并准备划分。
    支持 'full' (全量训练) 和 'subset' (10% 分层采样子集) 两种模式。
    在 subset 模式下，会先加载全量数据（从缓存或处理一次），然后根据标签分层采样，
    最后切割出真正的子集以加速后续预处理。
    """
    try:
    ...
    # 1. 加载完整数据集（从缓存读取或首次处理，这一步不可避免，但只做一次）
    dataset = PygPCQM4Mv2Dataset(root=dataset_dir)
    split_idx = dataset.get_idx_split()

    # 2. 划分固定验证集（从训练集中切出 150k）
    rng = default_rng(seed=42)
    train_idx_orig = rng.permutation(split_idx['train'].numpy())
    train_idx_orig = torch.from_numpy(train_idx_orig)
    valid_idx, train_idx = train_idx_orig[:150000], train_idx_orig[150000:]

    if name == 'full':
        split_idxs = [train_idx, valid_idx, split_idx['valid']]
    elif name == 'subset':
        # ---------- 分层采样 10% 训练集 ----------
        subset_ratio = 0.05
        # 取出训练样本的标签 (回归值)
        if hasattr(dataset.data, 'y'):
            all_labels = dataset.data.y.numpy().flatten()
            train_labels = all_labels[train_idx.numpy()]
        else:
            # 处理极少数 y 不在 data 中的情况（PCQM4Mv2 不会出现）
            train_labels = np.array([dataset[i].y.item() for i in train_idx])

        # 等频分桶（按百分位数将标签划分为 10 个桶，每个桶内样本数接近）
        num_bins = 10
        percentiles = np.linspace(0, 100, num_bins + 1)
        bins = np.percentile(train_labels, percentiles)
        bins[-1] += 1e-5  # 确保最大值能落在最后一个桶内
        bin_ids = np.digitize(train_labels, bins) - 1

        # 在每个桶内按比例采样
        sampled_parts = []
        for b in range(num_bins):
            idx_in_bin = train_idx.numpy()[bin_ids == b]
            if len(idx_in_bin) == 0:
                continue
            k = max(1, int(len(idx_in_bin) * subset_ratio))
            sampled = rng.choice(idx_in_bin, size=k, replace=False)
            sampled_parts.append(sampled)

        subtrain_idx = np.concatenate(sampled_parts)
        rng.shuffle(subtrain_idx)
        subtrain_idx = torch.from_numpy(subtrain_idx)
        logging.info(f"Stratified sampled train set: {len(subtrain_idx)} graphs")

        # 验证集从 valid_idx 中随机取 50000（也可以做分层，但验证集通常不需要太大）
        subvalid_idx = valid_idx[:50000]
        # 测试集保持原样（官方 valid 作为 test）
        subtest_idx = split_idx['valid']

        # ---------- 关键：切出真正的子集 ----------
        all_indices = torch.cat([subtrain_idx, subvalid_idx, subtest_idx])
        dataset = dataset[all_indices]  # 返回新的 InMemoryDataset，只包含子图
