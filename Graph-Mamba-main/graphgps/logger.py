...
class CustomLogger(Logger):
    def __init__(self, *args, **kwargs):
        ...
        if self.name == 'test':
            ...
            self.best_rmse_epoch = -1
            self.best_rmse_stats = {}
    ...
    # task properties
    # Modify the task-specific methods to calculate AUC for each epoch
    ...
    def write_epoch(self, cur_epoch):
        ...
        if self.name == 'test':
            # 对于分类任务：跟踪最佳AUC
            if 'auc' in stats:
                current_auc = stats['auc']
                if current_auc > self.best_auc:
                    self.best_auc = current_auc
                    self.best_auc_epoch = cur_epoch
                    self.best_auc_stats = stats.copy()
                    # 直接在日志中打印最佳AUC更新信息
                    logging.info(f"🚀 新的最佳测试集AUC: {current_auc:.4f} (第{cur_epoch}轮)")

            # 对于回归任务：跟踪最佳RMSE
            if 'rmse' in stats:
                current_rmse = stats['rmse']
                if current_rmse < self.best_rmse:
                    self.best_rmse = current_rmse
                    self.best_rmse_epoch = cur_epoch
                    self.best_rmse_stats = stats.copy()
                    # 直接在日志中打印最佳RMSE更新信息
                    logging.info(f"🚀 新的最佳测试集RMSE: {current_rmse:.4f} (第{cur_epoch}轮)")
            ...
            if 'mae' in stats:
                ...
                if current_mae < self.best_mae:
                    ...
                    self.best_mae_epoch = cur_epoch
                    self.best_mae_stats = stats.copy()
                    # 直接在日志中打印最佳MAE更新信息
                    logging.info(f"🚀 新的最佳测试集MAE: {current_mae:.4f} (第{cur_epoch}轮)")
        ...
        # 如果是最后一轮，打印最佳测试集指标总结
        if hasattr(cfg, 'optim') and hasattr(cfg.optim, 'max_epoch'):
            if cur_epoch == cfg.optim.max_epoch and self.name == 'test':
                self._print_final_best_stats()
    ...
    def _print_final_best_stats(self):
        """在训练结束时打印最佳测试集指标结果"""
        logging.info("=" * 80)
        logging.info("🎯 训练结束 - 测试集最佳指标结果总结:")

        # 打印分类任务的最佳AUC
        if self.best_auc_epoch != -1:
            logging.info("📊 分类任务:")
            logging.info(f"   最佳轮次: {self.best_auc_epoch}")
            logging.info(f"   ⭐ 最佳AUC: {self.best_auc:.4f}")
            logging.info("   详细指标:")
            for key, value in self.best_auc_stats.items():
                logging.info(f"      {key}: {value}")

        # 打印回归任务的最佳RMSE
        if self.best_rmse_epoch != -1:
            logging.info("📈 回归任务 - 最佳RMSE:")
            logging.info(f"   最佳轮次: {self.best_rmse_epoch}")
            logging.info(f"   ⭐ 最佳RMSE: {self.best_rmse:.4f}")
            logging.info("   详细指标:")
            for key, value in self.best_rmse_stats.items():
                logging.info(f"      {key}: {value}")

        # 打印回归任务的最佳MAE
        if self.best_mae_epoch != -1:
            logging.info("📉 回归任务 - 最佳MAE:")
            logging.info(f"   最佳轮次: {self.best_mae_epoch}")
            logging.info(f"   ⭐ 最佳MAE: {self.best_mae:.4f}")
            logging.info("   详细指标:")
            for key, value in self.best_mae_stats.items():
                logging.info(f"      {key}: {value}")
...
def eval_spearmanr(y_true, y_pred):
    ...
    if y_true.ndim == 1:
        ...
        for i in range(y_true.shape[1]):
            ...
            # Check if y_true is actually 1D after masking
            if is_labeled.ndim == 1:
                res_list.append(stats.spearmanr(y_true[is_labeled], y_pred[is_labeled])[0])
            else:
                # If is_labeled is not 1D, we need to flatten it before indexing
                flat_is_labeled = is_labeled.flatten()
                res_list.append(stats.spearmanr(y_true[flat_is_labeled], y_pred[flat_is_labeled])[0])
